...
- **2026-09-13 性能专项（本轮，工作区未提交）**：
  ...
  - **CI 复核（GitHub runner，已推送）**：新增 `.github/workflows/kmp-verify.yml`，不读任何 Secrets，三个 job 全部通过 —— run [34761597412](https://github.com/LiBigYe/BJTUselfService-KMP-Refreshed-err_li/actions/runs/34761597412)。`Shared tests (JVM)` 3m27s，测试报告 **96 个 suite / 476 项测试 / 0 失败 / 0 错误 / 0 跳过**；`Android assembleDebug (throwaway key)` 5m54s，产出 58.9MB debug APK（一次性调试钥匙，不是发布签名）；`Windows desktop compile` 5m44s（`:windowsApp:compileKotlinWindows` + `:desktopApp:compileKotlin`）。**这补上了本机无法验证的 Android 目标编译。**
...
- **fork 上没有 Secrets，旧的打包工作流未注册**：`LiBigYe/BJTUselfService-KMP-Refreshed-err_li` 的 Actions secrets 为 0，`gh workflow list` 只有 `KMP verify`。`kmp-package.yml`（APK/MSI/DMG/IPA + Release）需要 4 个签名 secret 才会生效，且该工作流在 fork 上尚未注册，需要在 Actions 页手动启用。
